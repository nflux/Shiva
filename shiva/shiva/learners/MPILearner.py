...
class MPILearner(Learner):
    ...
    def receive_trajectory_numpy(self, env_comm):
        ...
        if env_comm.Iprobe(source=MPI.ANY_SOURCE, tag=Tags.trajectory_info, status=self.info):
            ...
            self.log(f"Obs {observations.shape}\n {observations}", verbose_level=3)
            self.log(f"Acs {actions.shape}\n {actions}", verbose_level=3)
            self.log(f"AcsMask {actions_mask.shape}\n {actions_mask}", verbose_level=3)
            self.log(f"NextAcsMask {next_actions_mask.shape}\n {next_actions_mask}", verbose_level=3)
