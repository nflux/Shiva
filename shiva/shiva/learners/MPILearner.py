...
class MPILearner(Learner):
    ...
    def _receive_trajectory_numpy(self):
        ...
        self.traj_info = self.envs.recv(None, source=MPI.ANY_SOURCE, tag=Tags.trajectory_info,status=self.info)
    ...
    def update(self):
        if self.done_count > 0 and self.done_count % self.episodes_to_update == 0:
