...
class SingleAgentRPCLearner(Learner):
    def __init__(self, learner_id, config):
        super(SingleAgentRPCLearner ,self).__init__(learner_id, config)
    ...
    def step(self):
        ...
        if len(observation.shape) > 1:
            ...
            action = np.array(self.agent.get_action(observation, self.env.step_count))
            next_observation, reward, done, more_data = self.env.step(action)
