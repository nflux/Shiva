...
class MultiAgentTensorBuffer(ReplayBuffer):
    ...
    def sample(self, agent_id=None, device='cpu'):
        ...
        return (
            cast_obs(self.obs_buffer[inds, :, :].double()),
            cast(self.acs_buffer[inds, :, :].double()),
            cast(self.rew_buffer[inds, :, :].double()),
            cast_obs(self.next_obs_buffer[inds, :, :].double()),
...
class MultiAgentDaggerTensorBuffer(ReplayBuffer):
    ...
    def push(self, exps):
        ...
        if self.current_index + nentries > self.max_size:
            ...
            self.done_buffer = bh.roll(self.done_buffer, rollover)
            self.next_obs_buffer = bh.roll(self.next_obs_buffer, rollover)
            self.expert_acs_buffer = bh.roll(self.expert_acs_buffer, rollover)
            self.current_index = 0
            self.size = self.max_size
            # print("Roll!")
    ...
    def sample(self, agent_id=None, device='cpu'):
        ...
        return (
            ...
            cast_obs(self.next_obs_buffer[inds, :, :].double()),
            cast(self.done_buffer[inds, :, :]),
            cast(self.expert_acs_buffer[inds, :, :].double())
        )
