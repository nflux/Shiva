...
class SingleAgentTensorBuffer(ReplayBuffer):

    def __init__(self, max_size, batch_size, num_agents, obs_dim, acs_dim):
        super(SingleAgentTensorBuffer, self).__init__(max_size, batch_size, num_agents, obs_dim, acs_dim)
...
class MultiAgentTensorBuffer(ReplayBuffer):
    ...
    def sample(self, device='cpu'):
        ...
        return tuple(map(torch.squeeze, (
                    cast_obs(self.obs_buffer[inds, :, :]),
                    cast(self.acs_buffer[inds, :, :]),
