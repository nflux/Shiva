...
class TimeProfiler:
    ...
    def __init__(self, configs, base_dir, postfix_dir=''):
        self.configs = configs
        self._base_dir = base_dir
        self._save_dir = dh.make_dir(os.path.join(self._base_dir, 'profiler', postfix_dir), use_existing=True)
        if self.configs['Admin']['profiler']:
            self.writer = SummaryWriter(logdir=self._save_dir)
        self.reset()
    ...
    def _record(self, metric_name, x_value):
        ...
        # })
        if self.configs['Admin']['profiler']:
            # self.writer.add_scalar("Profiler/{}_per_second".format(metric_name), self._output_per_second, x_value)
            self.writer.add_scalar("Profiler/{}_per_minute".format(metric_name), self._output_per_minute, x_value)
            # self.writer.add_scalar("Profiler/{}_per_hour".format(metric_name), self._output_per_hour, x_value)
