...
class MADDPGAlgorithm(Algorithm):
    def __init__(self, observation_space: Dict[str, int], action_space: Dict[str, Dict[str, Tuple[Union[int]]]], configs: Dict[str, Any]) -> None:
        ...
        '''
        self.critic_input_size = sum([sum(self.action_space[role]['acs_space']) + self.observation_space[role] for role in self.roles])
    ...
    def update_permutes(self, agents: list, buffer: object, step_count: int, episodic=False):
        ...
        for perms_ix, perms in enumerate(possible_permutations):
            ...
            if buffer.prioritized:
                ...
                self._metrics[agent.id] += [('Algorithm/Weighted_Loss', critic_loss.item(), self.num_updates)]
            else:
                critic_loss = l2_error.mean()
                self._metrics[agent.id] += [('Algorithm/L2_Error', critic_loss.item(), self.num_updates)]
