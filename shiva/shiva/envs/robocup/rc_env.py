...
class rc_env:
    ...
    def __init__(self, config):
        ...
        if self.action_level == 'low':
            ...
            self.kick_actions = [hfo_env.DRIBBLE, hfo_env.SHOOT,
                                 hfo_env.PASS]  # actions that require the ball to be kickable
        ...
        for i in range(self.num_left):
            ...
            if i == 0:
                t = threading.Thread(target=self.connect, args=(self.port, self.feature_level, self.left_base,
                                                                self.goalie, i, self.ep_length, self.action_level,
                                                                self.left_envs,))
            else:
                t = threading.Thread(target=self.connect, args=(self.port, self.feature_level, self.left_base,
    ...
    def getReward(self, s, agentID, base, ep_num):
        ...
        if self.left_base == base:
            ...
            self.right_kickable[agentID] = self.get_kickable_status(agentID,
                                                                    env)  # update kickable status (it refers to previous timestep, e.g., it WAS kickable )
