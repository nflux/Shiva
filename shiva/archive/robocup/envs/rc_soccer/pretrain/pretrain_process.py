...
class pretrain:
    ...
    def __init__(self, config, env):
        ...
        self.pt_log_dir = config.pt_log_dir
    ...
    def pretraining(self):  
        if self.pt or self.use_pt_data:
            # ------------------------------------ Start Pretrain --------------------------------------------
            self.pt_trbs = [ReplayBufferLSTM(self.config,self.obs_dim,self.prox_item_size,self.pt) for _ in range(self.numBuffs)]
            self.pt_orbs = [ReplayBufferLSTM(self.config,self.obs_dim,self.prox_item_size,self.pt) for _ in range(self.numBuffs)]
    ...
    def load_PT_Buffers(self) :
            for i in range(1,self.numBuffs+1):
                ...
                print(os.getcwd() + '/' + self.pt_log_dir + '/pt_logs_%i' % ((i+1)*1000))

                if os.path.isdir(os.getcwd() + '/' + self.pt_log_dir + '/pt_logs_%i' % ((i+1)*1000)):
                    team_files = os.listdir(os.getcwd() + '/' +  self.pt_log_dir + '/pt_logs_%i' % ((i+1)*1000))
                    left_files = [os.getcwd() + '/' +  self.pt_log_dir + '/pt_logs_%i/' % ((i+1)*1000) + f for f in team_files if '_left_' in f]
            ...
            second_args = [(self.num_TA,self.obs_dim,self.acs_dim,self.pt_trbs[i],self.pt_orbs[i],self.episode_length,self.n_steps,self.gamma,self.D4PG,self.SIL,self.k_ensembles,self.push_only_left,self.LSTM_policy,self.prox_item_size) for i in range(self.numBuffs)]
            
            with Pool() as pool:
                pt_rbs = pool.starmap(misc.load_buffer, zip(self.all_l,self.all_r,self.all_s, second_args))
            self.pt_trbs,self.pt_orbs = list(zip(*pt_rbs))
            
            del pt_rbs
            
            for i in range(self.numBuffs):
                print("Length of RB_",i,": ",len(self.pt_trbs[i]))
                
            #team_replay_buffer = pt_trbs[0]
            #opp_replay_buffer = pt_orbs[0]
            
            self.pt_trb = ReplayBufferLSTM(self.config,self.obs_dim,self.prox_item_size,self.pt)
            self.pt_orb = ReplayBufferLSTM(self.config,self.obs_dim,self.prox_item_size,self.pt)
            for j in range(self.numBuffs):
                if not self.LSTM_policy:
                    self.pt_trb.merge_buffer(self.pt_trbs[j])
                    self.pt_orb.merge_buffer(self.pt_orbs[j])
                else:
                    self.pt_trb.merge_buffer(self.pt_trbs[j])
                    self.pt_orb.merge_buffer(self.pt_orbs[j])
                ...
                for ensemble in range(self.k_ensembles):
                    ...
                    for up in range(self.number_of_updates):
                        ...
                        if not self.load_same_agent:
                            inds = self.pt_trb.get_PER_inds(agentID,self.batch_size,self.ensemble)
                        else:
                            if up % batches_to_sample == 0:
                                m = 0
                                #inds = pt_trb[rb_i].get_PER_inds(m,batches_to_sample*batch_size,ensemble)
                                inds = np.random.choice(np.arange(len(self.pt_trb)), size=self.batch_size*batches_to_sample, replace=False)
                        ...
                        if LSTM:
                            ...
                            team_sample = self.pt_trb.sample(inds[self.batch_size*offset:self.batch_size*(offset+1)],
                        ...
                        if SIL:
                            for i in range(self.SIL_update_ratio):
                                inds = self.pt_trb.get_SIL_inds(agentID=m,batch_size=self.batch_size)
                                team_sample = self.pt_trb.sample(inds,
                                                        to_gpu=self.to_gpu,norm_rews=False)
                                opp_sample = self.pt_orb.sample(inds,
                                                        to_gpu=self.to_gpu,norm_rews=False)
                                priorities = mad_algo.SIL_update(team_sample, opp_sample, agentID, 'team') # 
                                self.pt_trb.update_SIL_priorities(agentID=m,inds = inds, prio=priorities)
                    #for c,p in enumerate(priorities):
