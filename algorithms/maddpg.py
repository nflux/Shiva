...
class RMADDPG(BASE_MADDPG):
    ...
    def update(self, team_sample=[], opp_sample=[], agent_i=0,logger=None,load_same_agent=False,critic=False,policy=True,session_path=""):
        ...
        if critic:
            ...
            critic_input = torch.cat((torch.cat((obs[0][:,:,:-self.num_out_pol],torch.stack(([obs[1 + i][:,:,26] for i in range (nagents-1)]),dim=2)),dim=2),torch.cat(acs,dim=2),torch.cat(opp_acs,dim=2)),dim=2)
            burn_in_tensor = critic_input[:self.lstm_burn_in]
            ...
            slice_obs = list(map(lambda x: x[self.lstm_burn_in:], obs))
            slice_acs = list(map(lambda x: x[self.lstm_burn_in:], acs))
            slice_opp_obs = list(map(lambda x: x[self.lstm_burn_in:], opp_obs))
            slice_opp_acs = list(map(lambda x: x[self.lstm_burn_in:], opp_acs))
            n_step_rews = list(map(lambda x: x[self.lstm_burn_in:], n_step_rews)) # extend dones and rewards using repeat
            MC_rews = list(map(lambda x: x[self.lstm_burn_in:], MC_rews))
            dones = list(map(lambda x: x[self.lstm_burn_in:], dones))
            next_obs = list(map(lambda x: torch.cat((x[self.lstm_burn_in:],x[-1:, :, :]),dim=0), obs))
            opp_next_obs = list(map(lambda x: torch.cat((x[self.lstm_burn_in:],x[-1:, :, :]),dim=0), opp_obs))
            start = time.time()

            if self.TD3:
                noise = processor(torch.randn_like(acs[0][self.lstm_burn_in-1:]),device=self.device,torch_device=self.torch_device) * self.TD3_noise
                if self.I2A:
            ...
            mod_next_obs = torch.cat((next_obs[0][:,:,:-self.num_out_pol],torch.stack(([next_obs[1 + i][:,:,26] for i in range (nagents-1)]),dim=2)),dim=2)
            ...
            mod_obs = torch.cat((slice_obs[0][:,:,:-self.num_out_pol],torch.stack(([slice_obs[1 + i][:,:,26] for i in range (nagents-1)]),dim=2)),dim=2)
        ...
        if policy:
            ...
            slice_obs = list(map(lambda x: x[self.lstm_burn_in:], obs))
            slice_acs = list(map(lambda x: x[self.lstm_burn_in:], acs)) # Not used currently
            slice_opp_obs = list(map(lambda x: x[self.lstm_burn_in:], opp_obs))
            slice_opp_acs = list(map(lambda x: x[self.lstm_burn_in:], opp_acs))
            ...
            obs_vf_in = torch.cat((slice_obs[0][:,:,:-self.num_out_pol],torch.stack(([slice_obs[1 + i][:,:,26] for i in range (nagents-1)]),dim=2)),dim=2)
        ...
        if policy:
            if self.niter % 100 == 0:
                print("Agent (%i) Actor loss:" % agent_i,pol_loss)

                if self.I2A:
                    print("Agent(%i) Policy Prime loss" % agent_i,pol_prime_loss)
                    print("Agent(%i) Environment Model loss" % agent_i,EM_loss)
    ...
    def pretrain_critic(self, team_sample=[], opp_sample =[], agent_i = 0, side='team',logger=None,load_same_agent=False,session_path="",lstm_burn_in=40):
        """
        Update parameters of agent model based on sample from replay buffer
        Inputs:
            sample: tuple of (observations, actions, rewards, next
        ...
        mod_obs = torch.cat((obs[0][:,:,:-self.num_out_pol],torch.stack(([obs[1 + i][:,:,26] for i in range (nagents-1)]),dim=2)),dim=2)
    ...
    def pretrain_actor(self, team_sample=[], opp_sample =[], agent_i = 0, side='team',logger=None,load_same_agent=False,session_path="",lstm_burn_in=40,burnin=False):
        """
...
# not implemented recurrent critic
class RCMADDPG(BASE_MADDPG):
    def __init__(self, env, config, only_policy = False):
        self = super().init_from_env(env, config, only_policy)
    ...
    def update(self, team_sample=[], opp_sample=[], agent_i=0, side='team', logger=None,
                                        load_same_agent=False,critic=False,policy=True,session_path=""):
        """
        Update parameters of agent model based on sample from replay buffer
        Inputs:
            sample: tuple of (observations, actions, rewards, next
...
# not implemented Reccurent Actor
class RAMADDPG(BASE_MADDPG):
    def __init__(self, env, config, only_policy = False):
        self = super().init_from_env(env, config, only_policy)
    ...
    def update(self, team_sample=[], opp_sample=[], agent_i=0, side='team', logger=None,
                                        load_same_agent=False,critic=False,policy=True,session_path="",lstm_burn_in=40):
        """
        Update parameters of agent model based on sample from replay buffer
        Inputs:
            sample: tuple of (observations, actions, rewards, next
...
class VanillaMADDPG(BASE_MADDPG):
    
    def __init__(self, env, config, only_policy = False):
        self = super().init_from_env(env, config, only_policy)
    
    def update_centralized_critic(self, team_sample=[], opp_sample =[], agent_i = 0, side='team', logger=None,load_same_agent=False,critic=True,policy=True,session_path=""):
        """
        Update parameters of agent model based on sample from replay buffer
        Inputs:
            sample: tuple of (observations, actions, rewards, next
    ...
    def pretrain_critic_MC(self, team_sample=[], opp_sample =[], agent_i = 0, side='team', logger=None,load_same_agent=False,session_path=""):
        """
        Update parameters of agent model based on sample from replay buffer
        Inputs:
            sample: tuple of (observations, actions, rewards, next
    ...
    def pretrain_actor(self, team_sample=[], opp_sample =[], agent_i = 0, side='team', logger=None,load_same_agent=False,session_path=""):
        """
        Update parameters of actor based on sample from replay buffer for policy imitation (fits policy to observed actions)
        Inputs:
            sample: tuple of (observations, actions, rewards, next
