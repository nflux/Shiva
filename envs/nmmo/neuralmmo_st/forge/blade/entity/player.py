...
class Player:
   public = set(
         'pos lastPos R C food water health entID annID name colorInd color timeAlive kill attackMap damage freeze immune teamID'.split())

   def __init__(self, entID, color, config):
      self._config = config

      self._entID = entID
      #self._name = ('Neural_' if entID != "0" else 'Player_') + str(self._entID)
      self._name = 'Procedural_' + str(self._entID)
      self._timeAlive = 0
      self._teamID = str(int(entID) % 2) # 2 teams

      self._R, self._C = config.R, config.C
      self._pos = config.SPAWN(self._entID, self._teamID)
      ...
      self._colorInd = int(self._teamID)
      self._attackMap = np.zeros((7, 7, 3)).tolist()

      self._index = 1
      self._immuneTicks = 0 #15 # 15 is original
   ...
   def updateStats(self):
      #TURNING OFF FOR COMBAT-ONLY SIMULATION
   ...
   def act(self, world, actions, arguments, val = None):
      if not self.alive: return
      self.mapAttack()
