...
Synthetic Training
Procedural control
Find nearest enemy and return the distance
"""
def findNearestEnemyAndDistance(world, entity, allEnts):
   distancesAndOpponents = list()
   for e in sortedByl1(allEnts, 80, entity):
      if e.teamID != entity.teamID:
         distOppTuple = (l1(e.pos, entity.pos), e)
         distancesAndOpponents.append(distOppTuple)
   if len(distancesAndOpponents) > 0:
      return distancesAndOpponents[0]
   else:
      return None
...
def sortedByl1(ent, sz, startEnt):
   ...
   targs = l1Range(ent, sz, startEnt.pos, 15)
   ...
   # for r in range(rt, rb):
      # for c in range(cl, cr):
         # if len(ent[r, c]) > 0:
            # ret += ent[r, c]
   for e in ent:
      if (rt <= e.pos[0] and rb >= e.pos[0]):
         if (cl <= e.pos[1] and cr >= e.pos[1]):
            ret.append(e)
   return ret
