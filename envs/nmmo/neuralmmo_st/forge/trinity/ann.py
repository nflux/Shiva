...
class AttackNet(nn.Module):
   ...
   def forward(self, env, ent, action, s):
      ...
      # # only target opposing team
      # print("[AttackNet][orig]targets=" + str(targets))
      # otherTeamTargets = list()
      # for targ in targets:
         # if ((ent.teamID != targ.teamID) or (ent == targ)):
            # otherTeamTargets.append(targ)
      # targets = otherTeamTargets
      # print("[AttackNet][team]targets=" + str(targets))
      # ### end target-based culling ###
      
      targets = torch.tensor([e.stim for e in targets]).float()
      #print("[AttackNet][1]targets=" + str(targets))
      targets = self.targEmbed(targets).unsqueeze(0)
      #print("[AttackNet][2]targets=" + str(targets))
      nTargs  = len(targets)
      ...
      #print("[AttackNet]attkOuts="+str(attkOuts))
      return action, [argument], attkOuts
