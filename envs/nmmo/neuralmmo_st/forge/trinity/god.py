...
class God:
   ...
   # Called after realm.spawn()
   def spawn(self, isBlue):
      #entID = str(self.entID)
      #annID = hash(entID) % self.nANN
      #self.entID += 1#set to 2 to test teams, else 1
      if (isBlue):
         self.entID = self.blueEntID
         self.blueEntID += 2
      else:
         self.entID = self.redEntID
         self.redEntID += 2
      entID = str(self.entID)
      annID = hash(entID) % self.nANN
      ...
      color = self.palette.color((int(entID) % 2)) # team colors
