...
class Player extends THREE.Object3D {
   ...
   //Format: pos = (r, c)
   coords(pos) {
      return new THREE.Vector3(pos[1]*sz+sz+sz/2, this.height, pos[0]*sz+sz+sz/2); // orig
      //return new THREE.Vector3((pos[1]+1)*sz+sz+sz/2, this.height, (pos[0]-1)*sz+sz+sz/2);
   }
   ...
   updateData(engine, packet, players) {
      ...
      if (packet['attack'] != null) {
         ...
         if ((this.entID != targID)
             && (targID in players)
             //&& ( (parseInt(this.entID) % 2) != ( parseInt(targID) % 2) ) 
             ) 
         {
            //console.log("ATTACK: " + this.entID + " attacks " + targID);
            var attk;
